...
let rendered = renderer.image { ctx in
    ...
    let buttomCircle = CGRect(x: 400, y: 700, width: 200, height: 200)
    ctx.cgContext.addEllipse(in: buttomCircle)
    ctx.cgContext.drawPath(using: .fillStroke)
    
    let rightCircle = CGRect(x: 100, y: 400, width: 200, height: 200)
    ctx.cgContext.addEllipse(in: rightCircle)
    ctx.cgContext.drawPath(using: .fillStroke)
    
    let leftCircle = CGRect(x: 700, y: 400, width: 200, height: 200)
